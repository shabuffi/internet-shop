...
def list_orders(
    ...
    Returns:
        Словарь с заказами (число позиций, признак гостя ``is_guest``), общим числом,
        номером страницы и размером страницы ``page_size`` (для подсчёта числа страниц на фронте).
    """
    ...
    return {
        ...
        "total": total,
        "page": page,
        # При фильтре по клиенту отдаём все его заказы одной страницей (page_size == total),
        # иначе — размер обычной страницы. Фронт по нему считает число страниц.
        "page_size": total if filtered else PAGE,
    }
