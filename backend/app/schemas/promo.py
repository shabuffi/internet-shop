...
class PromoCategoryAdmin(BaseModel):
    ...
    # Сколько попадёт в раздел: активные с галочкой в поле (остаток не учитываем — временный).
    product_count: int = 0
    # Сколько СЕЙЧАС видно на сайте: точное число со страницы раздела (активные, с остатком,
    # с учётом эксклюзивности). Может быть меньше product_count, если часть распродана.
    visible_count: int = 0
