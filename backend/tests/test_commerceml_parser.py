...
def test_parse_import_description_from_nested_full_name():
    """Реальный кейс МойСклад: описание в <ЗначенияРеквизитов> → «Полное наименование»."""
    xml = _import_xml(products=(
        "<Товар><Ид>p11</Ид><Артикул>PN-010</Артикул><Наименование>Ручка</Наименование>"
        "<ЗначенияРеквизитов>"
        "<ЗначениеРеквизита><Наименование>ВидНоменклатуры</Наименование><Значение>Товар</Значение></ЗначениеРеквизита>"
        "<ЗначениеРеквизита><Наименование>Полное наименование</Наименование><Значение>Гелевая ручка, 0,5 мм</Значение></ЗначениеРеквизита>"
        "</ЗначенияРеквизитов>"
        "</Товар>"
    ))
    p = parse_import_xml(xml).products[0]
    assert p.article == "PN-010"
    assert p.description == "Гелевая ручка, 0,5 мм"
...
def test_parse_import_property_names_and_values():
    """Свойства классификатора → catalog.property_names; значения товара → attributes.

    property_names — сигнал «пришла схема доп-полей»: по нему import_service решает,
    можно ли пересчитывать/сбрасывать флаги («Распродажа»/«Новинка»/«Убойные»).
    """
    xml = """<?xml version="1.0" encoding="UTF-8"?>
<КоммерческаяИнформация>
  <Классификатор>
    <Свойства>
      <Свойство><Ид>prop1</Ид><Наименование>Распродажа</Наименование></Свойство>
      <Свойство><Ид>prop2</Ид><Наименование>Количество в коробке</Наименование></Свойство>
    </Свойства>
  </Классификатор>
  <Каталог><Товары>
    <Товар><Ид>p1</Ид><Наименование>Товар</Наименование>
      <ЗначенияСвойств>
        <ЗначенияСвойства><Ид>prop1</Ид><Значение>1</Значение></ЗначенияСвойства>
      </ЗначенияСвойств>
    </Товар>
  </Товары></Каталог>
</КоммерческаяИнформация>""".encode()
    catalog = parse_import_xml(xml)
    assert catalog.property_names == {"Распродажа", "Количество в коробке"}
    assert {"name": "Распродажа", "value": "1"} in catalog.products[0].attributes
