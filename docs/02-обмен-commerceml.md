...
## ⭐ Главное, что путают: ДВА разных логин/пароль

В системе два набора учёток, и они работают в **противоположные стороны**.

### ① Логин/пароль обмена (МойСклад → магазин)

Это то, чем **МойСклад стучится в наш сайт**, чтобы залить каталог. В МойСклад
вписывается на экране «Онлайн-торговля → Адрес магазина». В нашей БД хранится в
`ShopSettings` под ключами `exchange_login` / `exchange_password`.
...
### ② Логин/пароль REST API (магазин → МойСклад)

Это то, чем **наш сайт стучится в МойСклад** — за картинками и чтобы отправлять
заказы. В БД: `moysklad_login` / `moysklad_password`. Здесь **только настоящие**
данные от аккаунта МойСклад, иначе картинки и заказы не работают.
См. [rest_client.py](../backend/app/integrations/moysklad/rest_client.py), функция `_get_credentials()`.

```
①  логин/пароль обмена   ← придуманные, лишь бы совпадали (вход В магазин)
②  логин/пароль REST     ← настоящие от МойСклад        (выход ИЗ магазина в МС)
```

## Протокол: 6 шагов по порядку
...
| 5 | `GET ?mode=import&filename=import.xml` | парсит оба XML, пишет в БД, чистит Redis | `success\nИмпортировано: ...` |
| 6 | `GET ?mode=import&filename=offers.xml` | offers уже учтён на шаге 5 — просто `success` | `success` |
