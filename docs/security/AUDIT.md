...
Последнее обновление: **2026-08-03** — добавлена безопасная смена email (см. B-3/B-4 ниже; в коде, НЕ задеплоено). Stages 1–3 **задеплоены на прод** 2026-07-14 (commit `9947863`, alembic head `d6e7f8a9b0c1`).
...
| B-3 | TOCTOU при регистрации: `users.email`/`phone` проверяются `select`-ом, вставка без гарда — гонка двух регистраций даёт 500 от unique-индекса | 🟢 Low | `api/v1/endpoints/auth.py` | ✅ fixed 2026-08-03 (`IntegrityError` → 409) |
| B-4 | Смена email как вектор захвата аккаунта (email = логин): реализована 2026-08-03 сразу с защитой — текущий пароль на заявку, подтверждение владения новым ящиком, письмо-сигнал на старый адрес, rate-limit по IP и по аккаунту | 🟢 Low | `api/v1/endpoints/auth.py` | ✅ fixed by design 2026-08-03 |
