  ...
  return (
    <AdminShell>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16, marginBottom: 40 }}>
        <StatCard icon="📦" label="Товаров" value={data?.product_count ?? "—"} />
        <StatCard icon="📋" label="Заказов" value={data?.order_count ?? "—"} />
        <StatCard icon="🔄" label="Последняя синхр." value={data?.last_sync?.status ?? "—"} />
      </div>
