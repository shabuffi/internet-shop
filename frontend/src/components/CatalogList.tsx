...
import { formatPrice, minOrderQty, minOrderQtyText } from "@/lib/format";
            ...
            {products.map((p) => (
              <tr key={p.id}>
                ...
                <td style={td}>
                  ...
                  <div style={{ fontSize: 12, fontWeight: 600, color: minOrderQty(p.attributes) ? "var(--accent)" : "var(--ink-tertiary)" }}>Мин. кол-во: {minOrderQtyText(p.attributes)}</div>
                </td>
          ...
          <div key={p.id} style={{ display: "flex", flexDirection: "column", gap: 10, padding: "12px 14px",
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
              <div style={{ fontSize: 12, color: "var(--ink-tertiary)", display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", minWidth: 0 }}>
                ...
                <span style={{ fontWeight: 600, color: minOrderQty(p.attributes) ? "var(--accent)" : "var(--ink-tertiary)" }}>Мин. кол-во: {minOrderQtyText(p.attributes)}</span>
