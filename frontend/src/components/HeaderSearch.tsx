...
function SearchForm({ initial, className, action, placeholder, hidden }: {
  initial: string; className: string; action: string; placeholder: string; hidden: [string, string][];
}) {
...
export default function HeaderSearch({ className = "", action }: { className?: string; action?: string }) {
  ...
  return (
    <Suspense fallback={<SearchForm initial="" className={className} action={target} placeholder={placeholder} hidden={[]} />}>
